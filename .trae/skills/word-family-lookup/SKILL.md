...
description: "Looks up words sharing the same root/word-family or similar morphology in the word-cards dataset via the project CLI (tools/words.js). Invoke before adding a new word, or when the user asks about related/derived words (e.g. construct vs construction). Read-only; do not edit files."
---
...
所有操作通过项目 CLI 完成，不要手工 grep/翻文件。工作目录为项目根目录。
...
4. 查询结果包含文件路径、family、morph 等，直接传给 word-update 或汇报给用户。

## 约束

- 本技能只读：不创建/修改/删除任何文件，不运行 add/update/build。
- 命令退出码非 0 时，按报错信息处理，不要猜测数据。
